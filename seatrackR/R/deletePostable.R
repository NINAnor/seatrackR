...
deletePostable <- function(delFiles){
  ...
  if(length(delFiles) != sumFilesExisting) stop(paste0(delFiles[!(delFiles %in% unlist(filesInPostable))], " not in postable!"))
...
deletePostable2 <- function(delFiles,
                            refreshView = TRUE){
  seatrackR:::checkCon()
